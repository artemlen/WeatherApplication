...
import { Text, View, ScrollView, Alert, Pressable, StatusBar } from 'react-native'
...
class HomeScreen extends React.Component { 
    ...
    state = {
      ...
      getWeather = async (latitude, longitude) => { 
        ...
        console.log(temp_max, temp_min, feels_like, pressure, name, humidity, visibility, sunrise_hour, sunrise_minute, sunset_hour, sunset_minute, speed);
        console.log(data1.data.daily[0]) 
      } 
      ...
      getLocation = async () => {
        try {
          ...
          Alert.alert('Невозможно определить местоположение', 'Пожалуйста перезагрузите приложение');
        }
      ...
      render () {
        ...
        return (
          ...
          />
        <Pressable style= {{}} onPress={() => this.props.navigation.navigate('CityChoice')}>
          <View style={{alignItems:'center', marginTop:'3%', marginBottom:'3%'}}>
              <Text style={{fontSize:18, fontWeight: '500'}}>Add City</Text>
          </View>
        </Pressable>
          </View>
        );
